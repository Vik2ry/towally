...
export class InvestorService {
    ...
    const transaction = await this.prisma.$transaction(async (prisma) => {
      try {
        ...
        if (action === 'BUY') {
          ...
          if (userData.wallyWallet < totalPrice) {
          ...
          await this.prisma.user.update({
            where: { id: userId },
            data: { wallyWallet: userData.wallyWallet - totalPrice },
          });

          // Update seller's balance
          await this.prisma.user.update({
            where: { id: share.ownerId },
            data: { wallyWallet: share.owner.wallyWallet + price },
          });
        ...
        if (action === 'SELL') {
          ...
          await this.prisma.user.update({
            where: { id: userId },
            data: { wallyWallet: userData.wallyWallet + price },
          });
    ...
    const transaction = await this.prisma.$transaction(async (prisma) => {
      try {
        ...
        if (action === 'BUY') {
          ...
          await this.prisma.user.update({
            where: { id: userId },
            data: { wallyWallet: userData.wallyWallet - amount },
          });
  ...
  private async getUser(userId: string) {
    try {
      ...
      console.log('userData of getUser: ', userData);
      if (!userData) {
